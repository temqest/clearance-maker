const FONT_FILES = {
  regularWoff2: "/fonts/BookmanOldStyle.woff2",
  regularWoff: "/fonts/BookmanOldStyle.woff",
  boldWoff2: "/fonts/BookmanOldStyle-Bold.woff2",
  boldWoff: "/fonts/BookmanOldStyle-Bold.woff",
  italicWoff2: "/fonts/BookmanOldStyle-Italic.woff2",
  italicWoff: "/fonts/BookmanOldStyle-Italic.woff",
...
import { formatEnglishDate, generateClearanceFilename } from "./formatters";
...
<div class="rtc-header">
  <img src="${origin}/assets/supreme-court-seal-left.png" alt="Supreme Court seal" class="rtc-header-logo" />
  ...
  <img src="${origin}/assets/regional-trial-court-seal-right.png" alt="Regional Trial Court seal" class="rtc-header-logo" />
</div>`;
}
...
<div class="rtc-bottom-wrap">
  ...
  <div class="rtc-bottom-main-row">
    <div class="rtc-or-block">
      <div class="rtc-or-row"><span class="rtc-or-label">O.R. No.</span><span class="rtc-or-colon">:</span><span class="rtc-or-value">${orNo}</span></div>
      <div class="rtc-or-row"><span class="rtc-or-label">Date</span><span class="rtc-or-colon">:</span><span class="rtc-or-value">${orDate}</span></div>
      <div class="rtc-or-row"><span class="rtc-or-label">C.T.C. No.</span><span class="rtc-or-colon">:</span><span class="rtc-or-value">${ctc}</span></div>
      ...
      <div class="rtc-applicant-signature-label">Signature of Applicant</div>
    </div>
    <div class="rtc-photo-box">${photoTag}</div>
    <div class="rtc-thumb-box">Right<br />Thumbmark</div>
  </div>
</div>`;
}
...
export async function buildActualDocumentHtml(formData, photoSrc = "", signatureSrc = "") {
  const res = await fetch("/assets/rtc-clearance-with-logo/RTCClearanceWithLogo.html");
  let content = await res.text();

  content = content.replaceAll('src="images/', 'src="/assets/rtc-clearance-with-logo/images/');
  content = applyMinimalData(content, formData);
  content = replaceHeaderBlock(content, formData);
  content = replaceGivenLine(content, formData);
  content = replaceFindingPurposeBlock(content, formData);
  content = replaceVerifiedBlock(content);
  content = replaceSignatoryBlock(content, formData, signatureSrc);
  content = replaceBottomBlock(content, formData, photoSrc);

  const fontPreloadLinks = `
<meta charset="UTF-8" />
<link rel="preload" href="${FONT_FILES.regularWoff2}" as="font" type="font/woff2" crossorigin="anonymous" />
<link rel="preload" href="${FONT_FILES.boldWoff2}" as="font" type="font/woff2" crossorigin="anonymous" />`;

  const headerFixCss = `
<style id="rtc-header-fix">
  @font-face {
    font-family: "RTCClearanceBookman";
    src: url('${FONT_FILES.regularWoff2}') format('woff2'),
         url('${FONT_FILES.regularWoff}') format('woff');
    font-style: normal;
    font-weight: 400;
    font-display: swap;
  }
  ...
  @font-face {
    font-family: "RTCClearanceBookman";
    src: url('${FONT_FILES.boldWoff2}') format('woff2'),
         url('${FONT_FILES.boldWoff}') format('woff');
    font-style: normal;
    font-weight: 700;
    font-display: swap;
  }
  ...
  @font-face {
    font-family: "RTCClearanceBookman";
    src: url('${FONT_FILES.italicWoff2}') format('woff2'),
         url('${FONT_FILES.italicWoff}') format('woff');
    font-style: italic;
    font-weight: 400;
    font-display: swap;
  }
  ...
  @font-face {
    font-family: "RTCClearanceBookman";
    src: url('${FONT_FILES.boldItalicWoff2}') format('woff2'),
         url('${FONT_FILES.boldItalicWoff}') format('woff');
    font-style: italic;
    font-weight: 700;
    font-display: swap;
  }
  ...
  const docTitle = `<title>${escapeHtml(generateClearanceFilename(formData))}</title>`;
  return content.replace("</head>", `${docTitle}${fontPreloadLinks}${headerFixCss}</head>`);
}
