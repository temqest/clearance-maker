...
export function MockProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // Try fetching live documents from Supabase & subscribe to Realtime updates
    let channel = null;
    try {
      ...
      if (supabase) {
          ...
          .then(({ data, error }) => {
            if (!error && data && data.length > 0) {
              const mappedDbDocs = data.map((row) => ({
                ...
                paymentNo: row.form_data?.paymentNo || `PAY-${String(row.id).slice(0, 6)}`,
                ...
                orNumber: row.or_number || row.form_data?.orNumber || "OR-ONLINE",
                ...
                photoSrc: row.form_data?.photoSrc || "",
              }));
        ...
        // Setup Realtime subscription for instant multi-user counter synchronization
        channel = supabase
            ...
            (payload) => {
              if (payload.new) {
                ...
                const syncedDoc = {
                  ...
                  orNumber: row.or_number || row.form_data?.orNumber || "OR-ONLINE",
                  amountPaid: row.form_data?.amountPaid || "₱150.00",
                  remarks: row.form_data?.remarks || "Realtime update",
                  documentType: row.form_data?.documentType || "Official Clearance Document",
                  photoSrc: row.form_data?.photoSrc || "",
                };
                ...
                setDocuments((prev) => {
                  const map = new Map();
                  map.set(syncedDoc.id, syncedDoc);
                  prev.forEach((d) => {
                    if (!map.has(d.id)) map.set(d.id, d);
                  });
                  return Array.from(map.values());
  ...
  /**
   * Async document lookup that queries in-memory state, localStorage, AND live Supabase DB
   */
  const lookupDocumentAsync = useCallback(async (query) => {
    if (!query || typeof query !== "string") return null;
    const cleanQuery = query.trim();
    if (!cleanQuery) return null;

    // 1. Try local memory state first
    const syncMatch = lookupDocument(cleanQuery);
    if (syncMatch) return syncMatch;

    // 2. Try explicit localStorage lookup in case submitted in another tab/window
    if (typeof window !== "undefined") {
      try {
        const stored = window.localStorage.getItem("clearance_app_documents");
        if (stored) {
          ...
          if (Array.isArray(parsed)) {
            ...
            const localMatch = parsed.find((doc) => {
              if (!doc) return false;
              const matchId = doc.id && doc.id.toLowerCase().includes(queryLower);
              const matchCertNo = doc.certNo && String(doc.certNo).toLowerCase().includes(queryLower);
              const matchPayment = doc.paymentNo && doc.paymentNo.toLowerCase().includes(queryLower);
              const matchOr = (doc.orNumber || doc.orNo) && String(doc.orNumber || doc.orNo).toLowerCase().includes(queryLower);
              const matchName = doc.fullName && doc.fullName.toLowerCase().includes(queryLower);
            ...
            if (localMatch) {
              setDocuments((prev) => [localMatch, ...prev.filter((x) => x.id !== localMatch.id)]);
              return localMatch;
            }
          }
        }
    ...
    // 3. Query live Supabase database
    try {
      const supabase = createSupabaseBrowserClient();
      if (supabase) {
        const { data, error } = await supabase
          .from("documents")
          .select("*")
          .or(`cert_no.ilike.%${cleanQuery}%,full_name.ilike.%${cleanQuery}%,or_number.ilike.%${cleanQuery}%`)
          .limit(5);
        ...
        if (!error && data && data.length > 0) {
          ...
          const fetchedDoc = {
            id: row.cert_no || row.id,
            certNo: row.cert_no ? row.cert_no.replace(/^DOC-/, "") : String(row.id),
            paymentNo: row.form_data?.paymentNo || `PAY-${String(row.id).slice(0, 6)}`,
            fullName: row.full_name || row.title || "ANONYMOUS",
            address: row.form_data?.address || "",
            purpose: row.purpose || row.form_data?.purpose || "General Purpose",
            ...
            orNumber: row.or_number || row.form_data?.orNumber || "OR-ONLINE",
            amountPaid: row.form_data?.amountPaid || "₱150.00",
            remarks: row.form_data?.remarks || "Fetched from database",
            documentType: row.form_data?.documentType || "Official Clearance Document",
            photoSrc: row.form_data?.photoSrc || "",
          };

          setDocuments((prev) => [fetchedDoc, ...prev.filter((d) => d.id !== fetchedDoc.id)]);
          return fetchedDoc;
        }
      }
    } catch (err) {}
  ...
  const submitUserDocument = async (docData) => {
    ...
    const newDoc = {
      ...
      createdAtTimestamp: docData.createdAtTimestamp || Date.now(),
    ...
    // Asynchronously insert to Supabase database with or_number and form_data
    try {
      const supabase = createSupabaseBrowserClient();
      if (supabase) {
        const { data, error } = await supabase
          ...
          .insert({
            ...
            cert_no: newDoc.id,
            or_number: newDoc.orNumber,
            form_data: newDoc,
          })
          .select();

        if (error) {
          console.warn("Supabase document insert warning:", error.message || error);
          dbError = error.message;
        } else if (data && data.length > 0) {
          console.log("Supabase document inserted successfully:", data[0].cert_no || data[0].id);
        }
      }
    } catch (err) {
