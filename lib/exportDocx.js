...
async function exportClearanceDocxFromTemplate(formData, fileName, assets = {}) {
  ...
  const replacements = [
    ...
    ["December 2, 1999", formatEnglishDate(formData.dob) || ""],
    ...
    ["Naga City, Cam. Sur", formData.issuedAt || "Iriga City"],
    ...
    ["Valid for 6 months from the date of issue.", formData.noteText || "Valid for 6 months from the date of issue."],
        ...
        children: [
          ...
          new TableCell({
            ...
            children: [
              ...
              paragraph([new TextRun({ text: data.courtEmail || "rtc1iriocc@judiciary.gov.ph", italics: true, size: 20 })], {
                alignment: AlignmentType.CENTER
              }),
...
export async function exportClearanceDocx(formData, fileName, assets = {}) {
  const effectiveFileName = fileName || generateClearanceFilename(formData);
  ...
  try {
    await exportClearanceDocxFromTemplate(formData, effectiveFileName, normalizedAssets);
    return;
  } catch {
  ...
  return generateRTCClearance({
    ...
    courtEmail: formData.courtEmail || "rtc1iriocc@judiciary.gov.ph",
